...
function validateCred(arr) {
  ...
  let toDouble = false;
  let sum = 0;
  let newDigits = Array(n).fill(null)
  while (i >= 0) {
    let cur = arr[i]
    if (toDouble) { cur *= 2 }
    if (cur > 9) { cur -= 9 }
    ...
    sum += cur;
    i--;
    // console.log({cur, sum, toDouble})
  }
  // console.log({arr, sum, newDigits})
  return sum % 10 === 0
}
...
function findInvalidCards(cards) {
  let out = []
  cards.forEach(card => {
    if (!validateCred(card)) { out.push(card) }
  })
  return out
}
...
function idInvalidCardCompanies(cards) {
  return Array.from(new Set(cards.map(card => compList[card[0]])))
}
